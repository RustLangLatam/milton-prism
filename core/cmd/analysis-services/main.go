...
import (
	...
	"milton_prism/core/services/analysis"
...
func main() {
	...
	if err = analysis.BuildAnalysisServer(ctx, newServices, grpcSrv); err != nil {
		log.Fatalf("Failed to create analysis server: %v", err)
	}
	...
	serverGroup := services.NewServerGroup(
		...
		grpcSrv,
		metricsSrv,
		anlsvcv1.RegisterAnalysisServiceHandlerFromEndpoint,
		"/health:analysis",
	)
