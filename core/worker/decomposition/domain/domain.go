...
type (
	...
	TargetTopology  = migrationv1.TargetTopology
)

// Topology constants re-exported so the application layer never imports the
// proto package directly (Canon dependency rule).
const (
	TopologyUnspecified   = migrationv1.TargetTopology_TARGET_TOPOLOGY_UNSPECIFIED
	TopologyMicroservices = migrationv1.TargetTopology_TARGET_TOPOLOGY_MICROSERVICES
	TopologyMonolith      = migrationv1.TargetTopology_TARGET_TOPOLOGY_MONOLITH
)
...
type JobPayload struct {
	...
	// RootSubdirectory mirrors the analysis scope: when set, stage 5 (contract
	// derivation) walks only this repository-relative subdirectory. Empty means
	// the whole repository root. Carried from the analysis pipeline so the same
	// monorepo scope is applied consistently to both workers.
...
// MonolithServiceName is the canonical name of the single service produced for
// a MONOLITH-topology migration. The whole monolith is regenerated as one
// HTTP-native service, so there is no per-domain split.
const MonolithServiceName = "app"

// CollapseToMonolith folds N characterised service candidates into a single
// candidate for a MONOLITH-topology migration. The single service owns every
// domain resource; because there is exactly one service, all inter-service data
// deps and operational couplings become internal calls and are dropped (they no
// longer cross a boundary). The collapsed candidate carries no Deps and no
// OperationalCouplings — there is nothing left to depend on.
//
// Returns nil when candidates is empty (no domain layer → no service to build);
// the no-boundaries path in the pipeline handles that case unchanged.
func CollapseToMonolith(candidates []ServiceCandidate, prefix string) []ServiceCandidate {
	if len(candidates) == 0 {
		return nil
	}
	...
	for _, c := range candidates {
		for _, r := range c.OwnedResources {
			if _, ok := seen[r]; ok {
				continue
			}
			seen[r] = struct{}{}
	...
	return []ServiceCandidate{{
		Name:           MonolithServiceName,
		ErrorPrefix:    prefix,
		OwnedResources: resources,
		// One service: no cross-service deps, no operational couplings remain.
		Deps:                 nil,
...
// boundary spec's `store:` field. Empty/unknown maps to "mongodb" (the original
// path, always generable) so an absent signal never breaks the generator; the
// concrete engine ("postgres"|"mysql") flows through verbatim. The generation
// worker's storeSection consumes the same label to branch the prompt.
...
// BuildMonolithBoundarySpecYAML generates the boundary spec for the single
// service of a MONOLITH-topology migration. It declares topology: monolith and
// api_gateway: false so the generator emits an HTTP-native service that is its
// own entry point — there is no separate gateway and no inter-service transport.
// The actual HTTP-native generator profile is a documented sub-pending; this
// spec is the contract the generator will consume once that profile exists.
func BuildMonolithBoundarySpecYAML(svc *migrationv1.ProposedService, store string) string {
	var b strings.Builder

	b.WriteString("# Milton Prism boundary spec — generated by decomposition engine\n")
	b.WriteString("# Topology: MONOLITH — single HTTP-native service, no API gateway.\n")
	b.WriteString("# The service exposes HTTP directly and is the entry point.\n\n")

	b.WriteString("name: " + svc.GetName() + "\n")
	b.WriteString("error_prefix: " + svc.GetErrorPrefix() + "\n")
	b.WriteString("topology: monolith\n")
	b.WriteString("api_gateway: false  # HTTP-native: the service is its own entry point\n")
	b.WriteString("transport: http  # native HTTP, not gRPC behind a gateway\n")
	b.WriteString("store: " + normalizeStore(store) + "  # persistence engine: postgres|mysql|mongodb\n")
	...
	b.WriteString("inter_service_deps: []  # single service — no inter-service deps\n")
	b.WriteString("shared_database: true  # single service owns the one database\n")
