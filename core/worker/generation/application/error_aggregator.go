...
func buildMessageErrorGo(varNames map[string]struct{}) string {
	...
	fmt.Fprint(w, "\tCode   string `json:\"code,omitempty\"`   // Domain error code (e.g. MIG107, ANL301) for client-side handling\n")
	fmt.Fprint(w, "}\n\n")
	...
	fmt.Fprint(w, "\t}\n\n")
	fmt.Fprint(w, "\t// Surface the domain error code (e.g. MIG107, ANL301) to the client so the\n")
	fmt.Fprint(w, "\t// panel can branch/localize on it. Only when a real \"CODE: message\" prefix was\n")
	fmt.Fprint(w, "\t// present and parts[0] looks like a domain code (avoids leaking arbitrary text\n")
	fmt.Fprint(w, "\t// from gRPC errors that merely happen to contain \": \").\n")
	fmt.Fprint(w, "\temittedCode := \"\"\n")
	fmt.Fprint(w, "\tif len(parts) > 1 && looksLikeErrorCode(code) {\n")
	fmt.Fprint(w, "\t\temittedCode = code\n")
	fmt.Fprint(w, "\t}\n\n")
	...
	fmt.Fprint(w, "\t\tCode:   emittedCode,\n")
	fmt.Fprint(w, "\t}\n")
	fmt.Fprint(w, "}\n\n")
	fmt.Fprint(w, "// looksLikeErrorCode reports whether s matches the domain error-code convention\n")
	fmt.Fprint(w, "// (2–5 uppercase letters followed by 2–4 digits, e.g. MIG107, ANL301, MIG222).\n")
	fmt.Fprint(w, "func looksLikeErrorCode(s string) bool {\n")
	fmt.Fprint(w, "\ti := 0\n")
	fmt.Fprint(w, "\tfor i < len(s) && s[i] >= 'A' && s[i] <= 'Z' {\n")
	fmt.Fprint(w, "\t\ti++\n")
	fmt.Fprint(w, "\t}\n")
	fmt.Fprint(w, "\tif i < 2 || i > 5 || i == len(s) {\n")
	fmt.Fprint(w, "\t\treturn false\n")
	fmt.Fprint(w, "\t}\n")
	fmt.Fprint(w, "\tdigits := 0\n")
	fmt.Fprint(w, "\tfor ; i < len(s); i++ {\n")
	fmt.Fprint(w, "\t\tif s[i] < '0' || s[i] > '9' {\n")
	fmt.Fprint(w, "\t\t\treturn false\n")
	fmt.Fprint(w, "\t\t}\n")
	fmt.Fprint(w, "\t\tdigits++\n")
	fmt.Fprint(w, "\t}\n")
	fmt.Fprint(w, "\treturn digits >= 2 && digits <= 4\n")
	fmt.Fprint(w, "}\n\n")
