...
- **Store is supported.** If `store` is anything other than `mongodb`, stop and report a **profile hole** (Python Profile A.10). Never improvise a PostgreSQL, MariaDB, or Redis adapter.
...
4. **Repositories** (Python Profile A.6). Implement ports against `AsyncIOMotorDatabase`. Add `system_counters` ID generation at the seed from the decomposition doc. Implement `MongoTransactionManager` if `needs_transaction`. Add soft-delete timestamp if `soft_delete: true`.
...
8. **Tests** (Python Profile A.8). All application tests use `MagicMock`/`AsyncMock` for ports — no Motor, no real MongoDB. Every use case: `≥1` success test + `≥1` error scenario. Every sentinel asserted by `≥1` test. Names: `test_<operation>_<scenario>`.
