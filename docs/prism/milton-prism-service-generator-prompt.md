...
2. **`milton-prism-go-profile.md`** — the Go + {MongoDB (§7) | PostgreSQL & MySQL/MariaDB via GORM (§7P)} + gRPC mechanisms.
...
rpcs:                             # must already exist in the proto; listed for traceability
  ...
  - UpdateFoo
  - DeleteFoo
store: mongodb                    # v1: mongodb (default) | postgres | mysql — see the "Persistence" section injected into this prompt
...
- **Store is supported.** Supported stores in v1 are `mongodb` (default → Go Profile §7), `postgres` and `mysql` (MySQL/MariaDB) (both → Go Profile §7P, **GORM ORM**). If a **"Persistence: … (GORM ORM)" section is injected into this prompt**, generate the GORM layer per §7P, selecting the driver by store (`gorm.io/driver/postgres` for `postgres`, `gorm.io/driver/mysql` for `mysql`). If `store` is anything else, stop and report a **profile hole** (Go Profile §15). Never improvise an unsupported adapter.
...
5. infrastructure/repositories/    store=mongodb → (mongo_<resource>_repository.go, mongo_transaction_manager.go, identifier.go);
                                     store=postgres|mysql → GORM models + (gorm_<resource>_repository.go, gorm_transaction_manager.go) + core/shared/gorm_client/builder.go (AutoMigrate, no migrations/*.sql)  [Go Profile §7P]
...
If this service needs a new shared infrastructure client that does not yet exist, follow Go Profile §9–§10 to add it to the `Services` container, but only for the supported stores (`mongodb` → §7/§10 mongo_client; `postgres`|`mysql` → §7P gorm_client) over `gRPC` — anything else (NATS, …) is a hole.
...
5. **Repositories.** Branch on `store`: for `mongodb` (default, Go Profile §7) implement the ports against `*mongo.Database`, add the compile-time interface check, the nil-safe transaction manager over `UseSession`, and the `system_counters` identifier generator. For `postgres`|`mysql` (Go Profile §7P, when the "Persistence: … (GORM ORM)" section is injected) implement the **same** ports with `gorm_<resource>_repository.go` over `*gorm.DB`, separate GORM model structs (in `infrastructure/repositories`) mapped to/from the domain types, a `gorm_transaction_manager.go` over `db.Transaction(...)`, autoincrement IDs and `gorm.DeletedAt` soft-delete, and `core/shared/gorm_client/builder.go` (driver by store; schema via `AutoMigrate`, no `migrations/*.sql`).
...
- An improvised NATS adapter instead of a stop report (PostgreSQL and MySQL/MariaDB are supported via GORM per §7P when the Persistence section is injected — for any OTHER unsupported store, stop and report).
